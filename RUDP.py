...
class RUDP:
    """Represents a socket for RUDP protocol."""
    ...
    def __init__(self, address, port):
        ...
        self.listen_thread = Thread(
            target=self.listenRUDP, name="listen_thread")
    ...
    def create_packet(self, seq_num, data):
        """Attach RUDP header to given data, ready to be sent via UDP."""
        length = len(data)
        ...
        # store checksum
        pkt[self.__checksum_start__:self.__checksum_end__] = hashlib.md5(
            pkt).digest()
        return bytes(pkt)

    def get_packet_info(self, packet):
        """Parse the incoming UDP packet data into RUDP packet seq_num and data"""
        packet = bytearray(packet)

        # validate checksum
        checksum = packet[self.__checksum_start__: self.__checksum_end__]
        packet[self.__checksum_start__: self.__checksum_end__] = bytearray(
            self.CHECKSUM_SIZE)
        assert hashlib.md5(packet).digest(
        ) == checksum, "Corrupted Packet: Different Checksum"
        ...
        data = bytes(
            packet[self.__checksum_end__: self.__checksum_end__ + packet_length])
        return seq_num, data
    ...
    def packet_recevied(self, packet, address):
        ...
        if not data:  # packet is an ACK
            ...
            print(
                f'Outside window: {self.__recv_start__}, Seq Num : {seq_num}')
